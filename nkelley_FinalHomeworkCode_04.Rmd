...
g <- ggplot(data = d, aes(x = x, y = y), xlab = "Brain Size Species Mean", ylab = "Max Longevity", main = "Brain Size vs Longevity")
g <- g + geom_point()
g <- g + geom_smooth(method = "lm", formula = y ~ x)
g <- g + geom_text(x = 200, y = 775, label = eq(x, y), parse = TRUE)
g
```

```{r model II regression analysis}
mII <- lmodel2(x ~ y, data = d, range.y = "relative", range.x = "relative",
    nperm = 1000)
mII
...
h <- ggplot(data = d, aes(x, y))
h <- h + geom_point()
h <- h + geom_smooth(method = "lmodel2", formula = y ~ x)
h <- h + geom_text(x = 200, y = 700, label = eq(x, y), parse = TRUE)
h
```
...
confint(m, 'Brain_Size_Species_Mean', level=0.90)
#                           5 %    95 %
...
pi <- predict(m, newdata = data.frame(Brain_Size_Species_Mean = 1.21799), interval = "prediction",
